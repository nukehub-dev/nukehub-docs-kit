...
export function Select({
  ...
  React.useEffect(() => {
    if (open && highlightedIndex >= 0) {
      const option = document.getElementById(`${listId}-option-${highlightedIndex}`);
  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      <button
        ...
        className={cn(
          "flex h-9 w-full items-center justify-between rounded-lg border border-input bg-input/80 px-3 py-1 text-left text-sm shadow-sm backdrop-blur-sm transition-colors",
        ...
        createPortal(
          <AnimatePresence>
            {open && (
              ...
              >
                <ul id={listId} role="listbox" className="max-h-60 space-y-1 overflow-auto p-1.5">

...
export function ThemeToggle({ className, variant = "dropdown" }: ThemeToggleProps) {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div
          className="absolute right-0 top-full z-50 mt-1.5 w-52 rounded-lg border border-border bg-popover p-2 shadow-lg"
          role="menu"
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        >
              ...
              return (
                <Tooltip key={swatch.name} content={swatch.label}>
                  <button
                    ...
                    className={cn(
                      "relative h-6 w-6 rounded-full border-2 transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-popover",

...
- **MDX shortcodes**: `Callout`, `Tabs`, `TabItem`, `FileTree`, `Mermaid`, `Steps`, `Step`, `YouTube`, `Odysee`, `ImageFigure`, `DataTable`, and `Citation` are registered in `DocLayout` and available in `.mdx` files. `Plotly` and `Model3D` are opt-in shortcodes provided by the kit; consumers enable them by installing the optional peer dependencies and passing the components to `DocLayout` via the `mdxComponents` prop.
- **Citations**: declare page references in frontmatter and cite them with `<Citation id="..." />`. Pass the `references` array to `DocLayout` and it renders a linked bibliography with copy-to-clipboard support for plain text, BibTeX, and RIS. `References` is also exported as a component for custom layouts.
